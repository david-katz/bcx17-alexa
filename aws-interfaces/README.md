...
This compiles the client into a jar located at `./target/kneebends-1.0.jar`. Note that this jar does not include any dependencies.
...
This will compile the client and package all dependencies into a jar located at `./target/kneebends-1.0.jar`.
...
```java
KneeBends client = KneeBends.builder()
...
```java
KneeBends client = KneeBends.builder().build();
client.shutdown();
...
## Making requests
After a client is configured and created, you can make a request to the service. A method on the client interface (`KneeBends`) is created for all actions (resource + method) in your API.
...
```java
KneeBends client = KneeBends.builder().build();
PostPushupDetectedResult result = client.postPushupDetected(new PostPushupDetectedRequest());
```
...
```java
KneeBends client = KneeBends.builder().build();
client.postPushupDetected(new PostPushupDetectedRequest().sdkRequestConfig(
...
Service exceptions and client exceptions can be handled separately. Any exceptions modeled in the API will be a subtype of KneeBendsException.
```java
try {
    client.postPushupDetected(...);
} catch(KneeBendsException e) {
   // All service exceptions will extend from KneeBendsException.
   // Any unknown or unmodeled service exceptions will be represented as a KneeBendsException.
